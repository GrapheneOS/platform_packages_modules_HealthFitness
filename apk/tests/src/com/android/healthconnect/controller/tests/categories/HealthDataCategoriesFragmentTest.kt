...
class HealthDataCategoriesFragmentTest {
  ...
  @BindValue
  val viewModel: HealthDataCategoryViewModel = Mockito.mock(HealthDataCategoryViewModel::class.java)
  ...
  fun categoriesFragment_isDisplayed() {
    Mockito.`when`(viewModel.categoriesData).then { MutableLiveData<List<HealthDataCategory>>(emptyList()) }
  ...
  fun categoriesFragment_withCategories_categoryInformationIsDisplayed() {
    Mockito.`when`(viewModel.categoriesData).then { MutableLiveData(
