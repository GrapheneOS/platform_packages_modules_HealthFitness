...
class LoadDeclaredHealthPermissionUseCaseTest {
    ...
    @Test
    fun invoke_returnsValidHealthPermissions() = runTest {
        val permissions =
            listOf(
                HealthPermission.fromPermissionString(HealthPermissions.READ_EXERCISE),
                HealthPermission.fromPermissionString(HealthPermissions.WRITE_HEIGHT),
            )

        healthPermissionReader.stub {
            on { getValidHealthPermissions(TEST_APP_PACKAGE_NAME) } doReturn permissions
        }
        ...
        assertThat((result as UseCaseResults.Success).data)
            .containsExactlyElementsIn(permissions.map { it.toString() })
    }

    @Test
    fun invoke_healthPermissionReaderError_returnsFailure() = runTest {
        healthPermissionReader.stub {
            on { getValidHealthPermissions(TEST_APP_PACKAGE_NAME) } doThrow
