...
class LoadEntriesViewModelTest {
    ...
    private val input =
        LoadEntriesInput(dataType = dataType, startTime = startTime, endTime = endTime)
    ...
    fun loadEntriesViewModel_requestsData_returnsErrorOnException() = runTest {
        ...
        mockLoadEntries.stub { onBlocking { load(input) } doThrow (securityException) }
    ...
    @Test
    fun loadEntriesViewModel_requestsDataWithSufficientPermissions_returnsListOfRecords() =
        runTest {
            val context: Context = ApplicationProvider.getApplicationContext()
            ...
            val weightRecord2 =
                WeightRecord.Builder(
                        GeneralUtils.getMetaData(context),
                        now,
                        Mass.fromGrams(65000.0),
                    )
                    .build()
            val recordList: List<Record> = listOf(weightRecord1, weightRecord2)
            mockLoadEntries.stub { onBlocking { load(input) } doReturn (recordList) }
