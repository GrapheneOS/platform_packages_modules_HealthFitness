...
class IntermenstrualBleedingFormatterTest {
    private val formatter = IntermenstrualBleedingFormatter()
    private val context: Context = ApplicationProvider.getApplicationContext()

    @Test
    fun formatIntermenstrualBleedingValue_returnsFormattedEntry() {
        val NOW: Instant = Instant.parse("2024-04-10T15:36:18.000Z")
        val record = IntermenstrualBleedingRecord.Builder(getMetaData(context), NOW).build()

        val formattedEntry = formatter.format(record, context)

        assertThat(formattedEntry.value).isEqualTo("Spotting")
    }
}
