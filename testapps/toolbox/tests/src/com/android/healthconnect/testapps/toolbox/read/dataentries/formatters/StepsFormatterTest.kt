...
class StepsFormatterTest {
    ...
    private val formatter = StepsFormatter()
    private val context: Context = ApplicationProvider.getApplicationContext()

    @Test
    fun formatStepsValue_returnedFormattedEntry() {
        val record = getStepsRecord(1221L)

        val formattedEntry = formatter.format(record, context)

        assertThat(formattedEntry.value).isEqualTo("1221 Steps")
    }

    private fun getStepsRecord(steps: Long): StepsRecord {
        val NOW: Instant = Instant.parse("2024-04-10T15:36:18.000Z")

        return StepsRecord.Builder(getMetaData(context), NOW, NOW.plusSeconds(1), steps).build()
    }
}
