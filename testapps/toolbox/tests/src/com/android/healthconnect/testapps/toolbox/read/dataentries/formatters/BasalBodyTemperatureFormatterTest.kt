...
class BasalBodyTemperatureFormatterTest {
    private val formatter = BasalBodyTemperatureFormatter()
    private val context: Context = ApplicationProvider.getApplicationContext()

    @Test
    fun formatBasalBodyTemperatureValue_returnsFormattedEntry_noMeasurementLocation() {
        val record = buildBasalBodyTemperatureRecord(MEASUREMENT_LOCATION_UNKNOWN, 33.0).build()

        val formattedEntry = formatter.format(record, context)

        assertThat(formattedEntry.value).isEqualTo("33.0 celsius ")
    }

    @Test
    fun formatBasalBodyTemperatureValue_returnsFormattedEntry_measurementLocation() {
        val record = buildBasalBodyTemperatureRecord(MEASUREMENT_LOCATION_TOE, 34.1).build()

        val formattedEntry = formatter.format(record, context)

        assertThat(formattedEntry.value).isEqualTo("34.1 celsius Toe")
    }

    private fun buildBasalBodyTemperatureRecord(
