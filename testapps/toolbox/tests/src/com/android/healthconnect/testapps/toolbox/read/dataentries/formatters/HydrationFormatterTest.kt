...
class HydrationFormatterTest {
    private val formatter = HydrationFormatter()
    private val context: Context = ApplicationProvider.getApplicationContext()

    @Test
    fun formatHydrationValue_returnsFormattedEntry() {
        val record = buildHydrationRecord(12.123).build()

        val formattedEntry = formatter.format(record, context)

        assertThat(formattedEntry.value).isEqualTo("12.123 L")
    }

    private fun buildHydrationRecord(volume: Double): HydrationRecord.Builder {
        val NOW: Instant = Instant.parse("2024-04-10T15:36:18.000Z")
        return HydrationRecord.Builder(
            getMetaData(context),
            NOW,
            NOW.plusSeconds(1),
            Volume.fromLiters(volume),
        )
    }
}
