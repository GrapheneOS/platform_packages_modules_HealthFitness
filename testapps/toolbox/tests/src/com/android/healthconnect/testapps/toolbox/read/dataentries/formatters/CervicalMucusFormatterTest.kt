...
class CervicalMucusFormatterTest {
    private val formatter = CervicalMucusFormatter()
    private val context: Context = ApplicationProvider.getApplicationContext()

    @Test
    fun formatCervicalMucusRecord_returnsFormattedEntry() {
        val record = getCervicalMucusRecord(SENSATION_LIGHT, APPEARANCE_DRY)

        val formattedEntry = formatter.format(record, context)

        assertThat(formattedEntry.value).isEqualTo("Dry Light")
    }

    private fun getCervicalMucusRecord(sensation: Int, appearance: Int): CervicalMucusRecord {
        val NOW: Instant = Instant.parse("2024-04-10T15:36:18.000Z")
        return CervicalMucusRecord.Builder(getMetaData(context), NOW, sensation, appearance).build()
    }
}
