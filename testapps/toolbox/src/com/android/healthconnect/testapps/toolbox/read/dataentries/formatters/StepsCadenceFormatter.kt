...
class StepsCadenceFormatter {
    ...
    fun format(record: StepsCadenceRecord, context: Context): FormattedDataDetails {
    ...
    private fun formatDataEntry(record: StepsCadenceRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value =
                "${getStepCadenceAverage(record.samples)} ${context.getString(R.string.steps_per_minute)}",
        )
    }

    private fun formatSamples(
        samples: List<StepsCadenceRecord.StepsCadenceRecordSample>,
        context: Context,
        ...
        samples.forEach {
            formattedSamples.add(
                FormattedSample(
                    ...
                    value = "${it.rate} ${context.getString(R.string.steps_per_minute)}",
                )
            )
        }
    ...
    private fun getStepCadenceAverage(
        samples: List<StepsCadenceRecord.StepsCadenceRecordSample>
    ): Double {
        return samples.sumOf { it.rate } / samples.size
    }
}
