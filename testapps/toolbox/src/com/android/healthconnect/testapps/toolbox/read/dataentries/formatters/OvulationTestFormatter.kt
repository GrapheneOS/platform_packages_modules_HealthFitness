...
class OvulationTestFormatter {
    ...
    fun format(record: OvulationTestRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value = formatTestResult(record.result, context),
        )
    }

    private fun formatTestResult(testResult: Int, context: Context): String {
        return when (testResult) {
            RESULT_HIGH -> context.getString(R.string.ovulation_test_high)
            RESULT_INCONCLUSIVE -> context.getString(R.string.ovulation_test_inconclusive)
            RESULT_NEGATIVE -> context.getString(R.string.ovulation_test_negative)
            RESULT_POSITIVE -> context.getString(R.string.ovulation_test_positive)
            else -> context.getString(R.string.ovulation_test_unknown)
        }
    }
}
