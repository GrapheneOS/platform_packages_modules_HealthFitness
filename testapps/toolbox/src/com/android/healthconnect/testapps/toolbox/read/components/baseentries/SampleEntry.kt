...
import com.android.healthconnect.testapps.toolbox.UIConstants.PADDING_XSMALL
import com.android.healthconnect.testapps.toolbox.read.dataentries.FormattedEntry.FormattedSample

@Composable
fun SampleEntry(sample: FormattedSample) {
    SampleEntry(header = sample.header, value = sample.value)
}

@Composable
fun SampleEntry(header: String, value: String) {
    Column(modifier = Modifier.padding(horizontal = PADDING_SMALL, vertical = PADDING_XSMALL)) {
