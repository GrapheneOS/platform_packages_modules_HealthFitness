...
class ElevationGainedFormatter {
    ...
    fun format(record: ElevationGainedRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value = UnitFormatter.formatLength(record.elevation, Unit.Length.KILOMETERS, context),
        )
    }
}
