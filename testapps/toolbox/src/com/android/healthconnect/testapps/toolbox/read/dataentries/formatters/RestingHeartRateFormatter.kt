...
class RestingHeartRateFormatter {
    ...
    fun format(record: RestingHeartRateRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value = "${record.beatsPerMinute} ${context.getString(R.string.beats_per_minute_label)}",
        )
    }
}
