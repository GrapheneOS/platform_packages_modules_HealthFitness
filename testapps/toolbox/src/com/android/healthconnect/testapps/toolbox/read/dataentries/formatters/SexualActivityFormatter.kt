...
class SexualActivityFormatter {
    ...
    fun format(record: SexualActivityRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value = formatProtection(record.protectionUsed, context),
        )
    }

    fun formatProtection(protection: Int, context: Context): String {
        return when (protection) {
            ...
            else -> context.getString(R.string.sexual_activity_unknown)
        }
    }
}
