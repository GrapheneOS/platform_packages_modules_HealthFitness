...
class BodyWaterMassFormatter {
    ...
    fun format(record: BodyWaterMassRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value = UnitFormatter.formatMass(record.bodyWaterMass, Unit.Mass.KILOGRAMS, context),
        )
    }
}
