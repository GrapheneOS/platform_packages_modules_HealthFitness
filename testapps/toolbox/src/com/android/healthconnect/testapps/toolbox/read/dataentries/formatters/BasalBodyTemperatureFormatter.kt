...
class BasalBodyTemperatureFormatter {
    ...
    fun format(record: BasalBodyTemperatureRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(header = getHeader(record), value = formatValue(record, context))
    }

    private fun formatValue(record: BasalBodyTemperatureRecord, context: Context): String {
        val measurementLocationFormatter = MeasurementLocationFormatter()

        return "${formatTemperature(record.temperature)}" +
            " ${measurementLocationFormatter.formatBodyTemperature(record.measurementLocation, context)}"
    }

    private fun formatTemperature(temperature: Temperature): String {
