...
class HeartRateVariabilityRmssdFormatter {
    ...
    fun format(record: HeartRateVariabilityRmssdRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value =
                "${record.heartRateVariabilityMillis} ${context.getString(R.string.milliseconds_label)}",
        )
    }
}
