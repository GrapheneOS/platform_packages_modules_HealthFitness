...
class FloorsClimbedFormatter {

    fun format(record: FloorsClimbedRecord, context: Context): FormattedDataEntry {
        return FormattedDataEntry(
            header = getHeader(record),
            value = "${record.floors} ${context.getString(R.string.floors)}",
        )
    }
}
