...
/**
 * A converter which takes a dex file and creates a jar containing all the classes, and methods and
 * fields stubbed out.
 */
public class DexToStubConverter {
    ...
    public String writeClass(DexBackedClassDef classDef) throws IOException {
        ...
        if ((classDef.getAccessFlags() & Opcodes.ACC_ANNOTATION) == Opcodes.ACC_ANNOTATION) {
            ...
            for (AnnotationElement ae : findAnnotation(classDef, ANNOTATION_DEFAULT_VALUE)) {
        ...
        for (DexBackedMethod method : classDef.getMethods()) {
            ...
            MethodParameter[] params =
                    method.getParameters().stream().toArray(MethodParameter[]::new);
            ...
            String descriptor =
                    Type.getMethodDescriptor(Type.getType(method.getReturnType()), paramTypes);
        ...
        for (DexBackedField field : classDef.getFields()) {
            ...
            Object value =
                    staticallyInitializedFields.contains(field.getName())
                            ? null
                            : getFieldValue(field);
    ...
    private static void collectTypeNames(
            Annotatable annotatable, String type, Collection<String> out) {
